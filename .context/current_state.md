...
- **`Header.svelte`**: A persistent header with a theme toggle.
- **`HeroSection.svelte`**: The main introductory section.
- **`AboutMeSection.svelte`**: A two-column section with finalized content.
- **`FeaturedProjectsSection.svelte`**: A responsive grid section to display project cards, ready for content.

### Key Constraint

- The project serves as a learning opportunity for `shadcn-svelte`, with a focus on overcoming self-identified weaknesses in UI design.
